// Upload the 2026-06 photo batch from data/ to the public "site-photos" bucket
// WITHOUT a service-role key. Auth model:
//   1. connect to Postgres with PG_PASSWORD (env only, never on disk)
//   2. create a temporary RLS policy letting the anon role insert/update ONLY
...
//
// Usage (PowerShell):
//   $env:PG_PASSWORD="<db password>"; node scripts/upload/upload_new_site_photos.mjs
...
const FILES = [
  "ali-mkumbwa-Annl9CjEaEs-unsplash.jpg",
  "austin-scherbarth-qSrFTyh-IB0-unsplash.jpg",
  "danist-soh-dqXiw7nCb9Q-unsplash.jpg",
  "emma-kapicha-cxiQkRowXw4-unsplash.jpg",
  "javier-miranda-MrWOCGKFVDg-unsplash.jpg",
  "omar-L8php9iH1pY-unsplash.jpg",
  "ricardo-gomez-angel-WsEAiVwGrwo-unsplash.jpg",
  "victor-0NJ9urGXrIg-unsplash.jpg",
  "vu-anh-TiVPTYCG_3E-unsplash.jpg",
  "zach-rowlandson-qhK0hRwKU60-unsplash.jpg",
];
...
const INS = "tmp_2026_06_photo_batch_ins";
const UPD = "tmp_2026_06_photo_batch_upd";

const client = await pickClient();
try {
  // Sanity: bucket must already exist (created by earlier upload runs).
  ...
  if (todo.length) {
    ...
    await client.query(`drop policy if exists ${UPD} on storage.objects`);
    await client.query(`
  ...
  await client.query(`drop policy if exists ${INS} on storage.objects`);
  await client.query(`drop policy if exists ${UPD} on storage.objects`);
  console.log("\ntemporary policies dropped — anon uploads closed again");

  const { rows: left } = await client.query(`
    select policyname from pg_policies
    where schemaname = 'storage' and tablename = 'objects'
      and policyname like 'tmp_2026_06_photo_batch%'`).catch(() => ({ rows: [] }));
  if (left?.length) console.error("WARNING: temp policies still present:", left);
